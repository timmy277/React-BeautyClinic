...
const GetInTouch = () => {
    const Container = tw.div`max-w-[64.875rem] mx-auto pt-[6.95rem] mb-[7.9rem]`
    ...
    const ListApproach = tw.div`flex items-center justify-between mt-[5.95rem]`
    const Approach = tw.div`text-center w-full`
    const Address = tw(Approach)`max-w-[16.875rem] mt-[0.7rem]`
    const Mail = tw(Address)``
    const Phone = tw(Approach)`max-w-[26.5rem] h-[25.125rem] pt-[4.938rem] shadow-contact rounded-[2.625rem]`

    const ApproachImgWrapper = tw.div`mb-[2.8rem]`
    const ApproachImg = tw.img`mx-auto`
    const ApproachSpan = tw(TwSpan)``
    const ApproachTitle = tw(TwTitle_MD)`text-2xl leading-[1.875rem] mx-auto my-[0.8rem] tracking-[0.014rem]`
    const ApproachTitlePhone = tw(ApproachTitle)`tracking-[0.006rem]`
    const ApproachDescription = tw(GrayP)`max-w-[16.875rem] mx-auto text-center text-sm leading-[1.313rem] font-normal tracking-[0.091rem] `
    return (
        <>
            <Container>
                <Introduction>
