...
const TreatmentVideo = () => {
    ...
    const VideoContainer = tw.div`max-w-[71.25rem] mx-auto z-10 absolute inset-0 pt-[10.7rem] md:pt-0 sm:pt-0 flex items-center justify-between 2lg:max-w-full 2lg:px-[10%] lg:max-w-full lg:px-[10%] mb-[8.5rem] md:flex-col md:px-[6%] md:max-w-full md:w-full sm:flex-col sm:px-[2%] sm:max-w-full sm:w-full`

    const VideoContent = tw.div`flex flex-col max-w-[50%] md:text-center md:max-w-full md:w-full md:mt-6 sm:text-center sm:max-w-full sm:w-full sm:mt-6`;
    const VideoTitle = tw(TwTitle_MD)`mb-[1.2rem] tracking-[0.015rem] text-white`;
    const VideoDescription = tw(BlueP)`mb-[2.5rem] text-dark_white tracking-[0.095rem] md:w-[80%] md:mx-auto md:mb-[1.5rem] sm:mb-[1rem] sm:w-[80%] sm:mx-auto`;

    const PlayButton = tw.div`flex items-center justify-between mt-[-2.2rem] pr-[3.8rem] 2lg:pr-[12%] lg:pr-[10%] md:pr-[8%] md:pt-[6%] md:w-full md:max-w-[60%] sm:pr-[6%]  sm:pt-[4%] sm:w-full sm:max-w-[50%]`;
    ...
    const Video = tw(GrayP)`text-white text-nowrap mb-[0.2rem]`;
    return (
        <>
            <TreatmentContainer>
