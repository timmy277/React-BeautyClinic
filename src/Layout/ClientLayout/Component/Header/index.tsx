    ...
    <header>
      <HeaderContainer>
        ...
        <div tw="top-[4rem] absolute right-[8rem] lg:right-[6rem] md:top-[2rem] sm:top-[2rem]">
          ...
          {openAccountManager && (
            <AccountManager>
              <Profile active={location.pathname === "/Profile"} feature={location.pathname === "/Profile"}>
                <Link to="/Profile" tw="flex items-center "><BsPersonVcard tw='mr-[0.5rem]' />Profile</Link>
              </Profile>
              <Bill active={location.pathname === "/Profile"} feature={location.pathname === "/Profile"}>
                <Link to="/Login" tw="flex items-center"><LuShoppingBasket tw='mr-[0.5rem]' />Bill</Link>
              </Bill>
                
              {userLoggedIn ? (
                <>
                  <Logout active={location.pathname === "/Logout"} feature={location.pathname === "/Logout"}
                    onClick={() => {
                      doSignOut().then(() => {
                        navigate("/Login");
                      });
                    }}>
                    <AiOutlineLogout tw='mr-2' />Logout
                  </Logout>
                </>
              ) : (
                <Logout active={location.pathname === "/Login"} feature={location.pathname === "/Login"}
                  onClick={() => {
                    navigate("/Login");
                  }}>Login
                </Logout>
